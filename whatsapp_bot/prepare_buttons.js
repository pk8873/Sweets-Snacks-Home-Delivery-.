...
const marker = "WHATSAPP_BUTTON_FLOW_PATCH_V5";

if (source.includes(marker)) {
  console.log("WhatsApp button/function compatibility patch V5 already applied.");
  process.exit(0);
}
...
replace(
  ...
  `const textOf=(m)=>{const x=unwrap(m?.message);return String(x.conversation||x.extendedTextMessage?.text||x.imageMessage?.caption||x.videoMessage?.caption||x.documentMessage?.caption||x.buttonsResponseMessage?.selectedDisplayText||x.templateButtonReplyMessage?.selectedDisplayText||x.listResponseMessage?.title||x.listResponseMessage?.description||x.interactiveResponseMessage?.body?.text||"").trim();};
...
replace(
  ...
  `async function buttons(sock,jid,text,items){const clean=items.filter(Boolean).slice(0,3);if(!clean.length)return sock.sendMessage(jid,{text});try{return await sock.sendMessage(jid,{text,footer:"Sweet & Snacks",buttons:clean.map(x=>({buttonId:x.id,buttonText:{displayText:x.text.slice(0,20)},type:1})),headerType:1,viewOnce:true})}catch(e){logger.warn({err:e},"Legacy buttons failed; native flow fallback");try{return await sock.sendMessage(jid,{text,footer:"Sweet & Snacks",interactiveButtons:clean.map(x=>({name:"quick_reply",buttonParamsJson:JSON.stringify({display_text:x.text.slice(0,20),id:x.id})}))})}catch{return sock.sendMessage(jid,{text:text+"\\n\\n"+clean.map((x,i)=>\`${i+1}. \${x.text}\`).join("\\n")})}}}
async function list`,
  "button sender",
);
...
replace(
  ...
  `async function list(sock,jid,text,rows,title="Choose"){const clean=rows.filter(Boolean).slice(0,10);if(!clean.length)return sock.sendMessage(jid,{text});try{return await sock.sendMessage(jid,{title:"🍬 Sweet & Snacks",text,footer:"Sweet & Snacks",buttonText:title,sections:[{title:"Options",rows:clean.map(x=>({rowId:x.id,title:x.title.slice(0,24),description:String(x.description||"").slice(0,72)}))}],viewOnce:true})}catch(e){logger.warn({err:e},"Legacy list failed; native flow fallback");try{return await sock.sendMessage(jid,{text,footer:"Sweet & Snacks",interactiveButtons:[{name:"single_select",buttonParamsJson:JSON.stringify({title,sections:[{title:"Options",rows:clean.map(x=>({title:x.title.slice(0,24),description:String(x.description||"").slice(0,72),id:x.id}))}]})}]})}catch{return sock.sendMessage(jid,{text:text+"\\n\\n"+clean.map((x,i)=>\`${i+1}. \${x.title}\`).join("\\n")})}}}
async function home`,
  "list sender",
);

replace(
  /async function product\(sock, jid, p\) \{[\s\S]*?\nasync function productActions/,
  `async function product(sock,jid,p){const s=state(jid);s.product=p;s.qty=Math.max(Number(s.qty||1),1);const f=await getFavorites(payload(jid,s.name));s.favorite=f.products.some(x=>x.id===p.id);if(p.is_weight_based)s.weight=Number(s.weight||p.weight_options?.[0]||250);return productActions(sock,jid,p)}
async function productActions`,
  "weight flow",
);
...
replace(
  ...
  `if(action.startsWith("weight_")){const [,pid,gt]=action.split("_"),r=await getProduct(Number(pid)),g=Number(gt);if(!r.product)return home(sock,jid);if(!(r.product.weight_options||[]).includes(g))return product(sock,jid,r.product);s.weight=g;s.qty=Math.max(Number(s.qty||1),1);return productActions(sock,jid,r.product)}
    if (action.startsWith("qtyinc_")`,
  "weight action",
);
...
replace(
  ...
  "stateless product branches",
);

replace(
  /syncFullHistory: false,\n  \}\);/,
  `syncFullHistory:false,patchMessageBeforeSending:(message)=>{if(!(message.buttonsMessage||message.listMessage))return message;return {viewOnceMessage:{message:{messageContextInfo:{deviceListMetadata:{},deviceListMetadataVersion:2},...message}}}},
  });`,
  "button envelope",
);
...
console.log("WhatsApp button/function compatibility patch V5 applied.");
