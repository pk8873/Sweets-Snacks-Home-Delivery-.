...
const marker = "WHATSAPP_RUNTIME_FIX_V69";
let source = fs.readFileSync(file, "utf8");

if (source.includes(marker)) {
  execFileSync(process.execPath, ["--check", path], { stdio: "inherit" });
  console.log("WhatsApp runtime V69 already applied; syntax check passed.");
  process.exit(0);
}

// V69 changes only the WhatsApp interactive transport compatibility layer.
// It does not change products, cart, orders, payments, delivery, Django APIs,
// Telegram, database models, or the existing action IDs/handlers.

const oldHelperImport = 'import baileysHelper from "zqbaileys_helper";';
const newHelperImport = 'import * as baileysHelperModule from "zqbaileys_helper";\nconst baileysHelper = baileysHelperModule.default || baileysHelperModule;';

if (source.includes(oldHelperImport)) {
  source = source.replace(oldHelperImport, newHelperImport);
}

if (!source.includes('from "zqbaileys_helper"')) {
  throw new Error("V69 could not locate zqbaileys_helper import.");
}
...
if (!source.includes("const { sendInteractiveMessage } = baileysHelper;")) {
  throw new Error("V69 could not enable sendInteractiveMessage.");
}
...
if (start < 0 || end < 0 || end <= start) {
  throw new Error("V69 could not locate the current buttons/list transport block.");
}
...
for (const required of [
  ...
  if (!finalSource.includes(required)) {
    throw new Error(`V69 validation failed: missing ${required}`);
  }
}

if (finalSource.includes("sendButtons") || finalSource.includes("sendListMessage")) {
  throw new Error("V69 validation failed: convenience interactive transport remains active.");
}

const helperModule = await import("zqbaileys_helper");
const helper = helperModule?.default || helperModule;
if (typeof helper?.sendInteractiveMessage !== "function") {
  throw new Error("V69 validation failed: zqbaileys_helper.sendInteractiveMessage is unavailable.");
}

console.log("WhatsApp runtime V69 applied; ESM/CJS helper compatibility + low-level native-flow transport + syntax validation passed.");
