...
import {
  addToCart, checkout, getCart, getCategories, getCustomer, getOrder,
  getOrders, getProducts, saveAddress,
} from "./api.js";
...
async function categories(sock, jid) {
  ...
  const s = session(jid); s.step = "category"; s.categories = r.categories;
  await sock.sendMessage(jid, { text: r.categories.length
    ? "🛍️ *SHOP*\n\n" + r.categories.map((c, i) => `${i + 1}️⃣ ${c.emoji} ${c.name}`).join("\n")
    : "😔 अभी कोई category उपलब्ध नहीं है।" });
}

async function products(sock, jid, category) {
  const r = await getProducts(category.id);
  const s = session(jid); s.step = "product"; s.category = category; s.products = r.products;
  ...
  for (let i = 0; i < r.products.length; i++) {
    ...
    if (p.image_url) {
      try { await sock.sendMessage(jid, { image: { url: p.image_url }, caption }); continue; }
      catch (e) { logger.warn({ e, id: p.id }, "Image send failed"); }
    }
...
async function addProduct(sock, jid, qty, grams = 0) {
  const s = session(jid); const p = s.product;
  if (!p) return;
...
async function cart(sock, jid) {
  const r = await getCart({ wa_id: jid, phone: phone(jid), display_name: session(jid).name });
  session(jid).step = "cart";
...
async function orders(sock, jid) {
  const r = await getOrders({ wa_id: jid, phone: phone(jid), display_name: session(jid).name });
  session(jid).step = "orders";
...
async function details(sock, jid, orderId) {
  const r = await getOrder({ wa_id: jid, phone: phone(jid), display_name: session(jid).name, order_id: orderId });
  const o = r.order;
...
async function beginCheckout(sock, jid) {
  const s = session(jid);
  const c = await customer({ wa_id: jid }, s.name);
  const r = await getCart({ wa_id: jid, phone: phone(jid), display_name: s.name });
  if (!r.items.length) return sock.sendMessage(jid, { text: "🛒 Cart खाली है। पहले product add करें।" });
  s.savedAddress = c.address;
  if (c.address) {
    s.step = "address_choice"; s.addressId = c.address.id;
    return sock.sendMessage(jid, { text: `📍 *DELIVERY ADDRESS*\n\n${c.address.address}, ${c.address.city} - ${c.address.pincode}\n\n1️⃣ Use saved address\n2️⃣ New address` });
  }
  s.step = "address";
  await sock.sendMessage(jid, { text: "📍 Address भेजें:\nHouse/Street, City, Pincode" });
}

async function payment(sock, jid) {
  const s = session(jid); s.step = "payment";
  await sock.sendMessage(jid, { text: "💳 Payment method चुनें:\n\n1️⃣ Cash on Delivery\n2️⃣ Online Payment" });
}

async function createOrder(sock, jid, method) {
  const s = session(jid);
  const r = await checkout({ wa_id: jid, phone: phone(jid), display_name: s.name, address_id: s.addressId, payment_method: method });
  s.step = "main";
  if (method === "online" && r.payment_url) {
    return sock.sendMessage(jid, { text: `🧾 *ORDER CREATED*\n\nOrder ID: *${r.order_id}*\nTotal: *₹${r.total}*\n\n💳 Payment:\n${r.payment_url}` });
  }
...
async function handle(sock, m) {
  ...
  const text = textOf(m); if (!text) return;
  const name = m.pushName || "Customer"; const s = session(jid, name); const lower = text.toLowerCase();
  ...
  try {
    ...
    if (lower.startsWith("track ")) {
      const r = await getOrder({ wa_id: jid, phone: phone(jid), display_name: name, order_id: text.slice(6).trim() });
      return sock.sendMessage(jid, { text: `📍 *ORDER TRACKING*\n\nOrder: *${r.order.order_id}*\nCurrent status: *${r.order.status}*\n\nयह current Admin status है।` });
    }
    if (lower.startsWith("details ")) return details(sock, jid, text.slice(8).trim());

    if (s.step === "category") {
      const i = Number(text) - 1; if (s.categories?.[i]) return products(sock, jid, s.categories[i]);
    }
    if (s.step === "product") {
      const i = Number(text) - 1; if (s.products?.[i]) return selectProduct(sock, jid, s.products[i]);
    }
    if (s.step === "weight") {
      const grams = ({ "1": 250, "2": 500, "3": 750, "4": 1000 })[text];
      if (grams) return addProduct(sock, jid, 1, grams);
    }
    if (s.step === "pieces") {
      const qty = Number(text); if (Number.isInteger(qty) && qty > 0) return addProduct(sock, jid, qty);
    }
    if (s.step === "address_choice") {
      if (text === "1") return payment(sock, jid);
      if (text === "2") { s.step = "address"; return sock.sendMessage(jid, { text: "📍 नया address भेजें: House/Street, City, Pincode" }); }
    }
    if (s.step === "address") {
      const parts = text.split(",").map((x) => x.trim()).filter(Boolean);
      if (parts.length < 3 || !/^\d{5,6}$/.test(parts.at(-1))) return sock.sendMessage(jid, { text: "❌ Format: House/Street, City, Pincode" });
      const pincode = parts.pop(); const city = parts.pop(); const address = parts.join(", ");
      const r = await saveAddress({ wa_id: jid, phone: phone(jid), display_name: name, address, city, pincode });
      s.addressId = r.address_id; return payment(sock, jid);
    }
    if (s.step === "payment") {
      if (text === "1" || lower === "cod") return createOrder(sock, jid, "cod");
      if (text === "2" || lower === "online") return createOrder(sock, jid, "online");
    }
    await sock.sendMessage(jid, { text: "मैं समझ नहीं पाया। `menu` भेजें।" });
  } catch (error) {
    logger.error({ error, jid }, "WhatsApp handler failed");
...
async function connect() {
  ...
  sock.ev.on("connection.update", ({ connection, lastDisconnect, qr }) => {
    if (qr) { console.log("📱 Scan this QR: WhatsApp > Linked Devices > Link a Device"); qrcode.generate(qr, { small: true }); }
...
http.createServer((req, res) => {
  if (req.url === "/" || req.url === "/health") { res.writeHead(200, { "Content-Type": "application/json" }); return res.end(JSON.stringify({ ok: true, service: "whatsapp-bot" })); }
  res.writeHead(404); res.end("Not found");
}).listen(PORT, () => console.log(`WhatsApp health server listening on ${PORT}`));
