...
const marker = "WHATSAPP_RUNTIME_FIX_V25";

let src = fs.readFileSync(path, "utf8");
if (src.includes(marker)) {
  console.log("WhatsApp runtime fix V25 already applied; nothing to do.");
  process.exit(0);
}
...
if (!src.includes(importLine) && !src.includes("generateWAMessageFromContent")) {
  throw new Error("Unable to find Baileys import for V25.");
}
if (src.includes(importLine)) src = src.replace(importLine, importReplacement);
...
const buttonsReplacement = `async function buttons(sock, jid, text, items) {
  ...
  try {
    ...
    const waMessage = generateWAMessageFromContent(jid, { interactiveMessage }, {
    ...
    await sock.relayMessage(jid, waMessage.message, {
      ...
      additionalNodes: String(jid).endsWith("@g.us") ? [bizNode] : [botNode, bizNode]
    });
  } catch (error) {
