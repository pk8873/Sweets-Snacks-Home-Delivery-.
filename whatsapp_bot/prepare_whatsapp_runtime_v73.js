...
const marker = "WHATSAPP_RUNTIME_FIX_V73";
let source = fs.readFileSync(file, "utf8");

if (source.includes(marker)) {
  execFileSync(process.execPath, ["--check", path], { stdio: "inherit" });
  console.log("WhatsApp runtime V73 already applied; syntax check passed.");
  process.exit(0);
}

// V73 is WhatsApp interactive transport only. It does not change any
...
if (!source.includes('from "zqbaileys_helper"')) {
  ...
  source = source.replace('import baileysHelper from "zqbaileys_helper";', helperImport);
}

if (!source.includes("const { sendInteractiveMessage } = baileysHelper;")) {
  const destructuringPattern = /const \{\s*sendButtons\s*,\s*sendListMessage\s*\}\s*=\s*baileysHelper;/;
  if (destructuringPattern.test(source)) {
    source = source.replace(destructuringPattern, 'const { sendInteractiveMessage } = baileysHelper;');
  } else {
    const anchor = 'const logger = P({ level: process.env.LOG_LEVEL || "info" });';
    if (!source.includes(anchor)) throw new Error("V73 could not locate logger anchor.");
...
if (start < 0 || end < 0 || end <= start) {
  throw new Error("V73 could not locate the WhatsApp buttons/list transport block.");
}
...
for (const required of [
  ...
  if (!finalSource.includes(required)) throw new Error(`V73 validation failed: missing ${required}`);
}

if (finalSource.includes('buttons: clean.map(x => ({ buttonId:') || finalSource.includes('buttonText: { displayText:')) {
  throw new Error("V73 validation failed: legacy button transport remains active.");
}
if (finalSource.includes('await sock.sendMessage(jid, { title: "🍬 Sweet & Snacks"')) {
  throw new Error("V73 validation failed: legacy list transport remains active.");
}

const helperModule = await import("zqbaileys_helper");
const helper = helperModule?.default || helperModule;
if (typeof helper?.sendInteractiveMessage !== "function") {
  throw new Error("V73 validation failed: zqbaileys_helper.sendInteractiveMessage is unavailable.");
}

console.log("WhatsApp runtime V73 applied; helper low-level native-flow transport enforced + legacy transport removed + syntax check passed.");
